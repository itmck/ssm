...
$(function(){
	...
	if($(".topMenu .d2 .dd").length){
		$(".topMenu .d2 .dd").append('<div class="client-promo"><a href="http://app.sfbest.com#trackref=sfbest_Uhead_Uhead_head_app2" target="_blank" rel="nofollow">先摇券 后买单</a></div>');
	}
	if($(".f_ios").length){
		$(".f_ios").find("li:first").html('<a target="_blank" href="http://app.sfbest.com">手机客户端</a>');
	}
	...
	if(ZnowTime >= 1414771200000 && ZnowTime<=1416239999000){
		$('.index_rfloat').html('<a href="/html/activity/1414059494.html" target="_blank"><img src="http://p.sfimg.cn/gold/images/20141113/20141113111801929.jpg"></a><div class="J_rclose">关闭</div>');
		$('.index_rfloat').show();
	}
...
function isOnline(wwwurl,homeurl,passporturl){
	$.getJSON( wwwurl+"/ajax/isOnline/?callback=?", function( data ) {
		...
		if (data.welcome){
		    ...
		    var loginUrl = passporturl+'/?returnUrl='+escape(document.location.href);
			var loginHtml='<a href="'+passporturl+'/reg/?returnUrl='+escape(document.location.href)+'">免费注册</a>';
			if(data.RegActiveName){
				loginHtml='<div class="newregbox"><a href="'+passporturl+'/reg/?returnUrl='+escape(document.location.href)+'">免费注册</a><div class="newregdiv ac_newregdiv"><span class="up-icon"></span><span class="newregTips"><em class="newwords">'+data.RegActiveName+'</em></span></div></div>';
			}
			$('#login').html(welComeMsg+'<a href="'+passporturl+'/?returnUrl='+escape(document.location.href)+'">请登录</a> | '+loginHtml);			
		    $('#product_kefu').attr('href',loginUrl);
		    $('#product_kefu').removeAttr('target');
		}
		if(data.qqcb){
			$('#qqcb').html(data.qqcb); 
		}
